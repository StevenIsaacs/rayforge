...
- This version does not include acceleration and deceleration power compensation. Because of this the ends of lines are engraved darker or burned deeper. A solution to this problem is coming.
...
- Vector acceleration and deceleration power compensation is implemented through min and max power ranges: non-overscan VECTOR layers declare and emit `power_range(min, power)` so the controller ramps laser power at the ends of straight lines, reducing burn. The lower bound is the layer's first workflow step `min_power`, resolved in precedence order step attribute, `step.extra`, then `layer.extra`, defaulting to the 8% controller floor. IMAGE, overscan, and raster layers are unaffected.
